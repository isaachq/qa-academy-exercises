...
test('[API-AUTH-001] Login returns the expected user and usable tokens', async ({ request }) => {
  await labels('API-AUTH-001 - Valid login');
  const response = await step('When: client logs in with the configured account', () =>
    request.post('/api/auth/login', {
      data: { email: environment.uiEmail, password: process.env.UI_PASSWORD },
    }));
...
test('[API-AUTH-002] Invalid credentials return a controlled unauthorized response', async ({ request }) => {
...
test('[API-AUTH-003] Authenticated profile belongs to the configured account', async ({ request }) => {
...
for (const credential of [
  ...
  test(`[API-AUTH-004] Protected profile rejects ${credential.name} Bearer`, async ({ request }) => {
...
test('[API-HEALTH-001] Authenticated health exposes known component checks', async ({ request }) => {
