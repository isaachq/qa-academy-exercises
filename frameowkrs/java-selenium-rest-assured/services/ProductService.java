...
public final class ProductService {
    ...
    public Map<String, Object> createProduct(Map<String, Object> product) {
        Response response = given().headers(headers()).contentType(ContentType.JSON).body(product)
    ...
    public Map<String, Object> getProduct(int id) {
        return given().headers(headers()).get(Environment.BASE_URL + "/api/products/" + id)
                .then().statusCode(200).extract().jsonPath().getMap("data");
    }

    public Map<String, Object> updateProduct(int id, Map<String, Object> changes) {
        return given().headers(headers()).contentType(ContentType.JSON).body(changes)
                .patch(Environment.BASE_URL + "/api/products/" + id)
                .then().statusCode(200).extract().jsonPath().getMap("data");
    }

    public void deleteOrder(int id) {
        int status = given().headers(headers()).delete(Environment.BASE_URL + "/api/orders/" + id)
                .statusCode();
        if (status != 200 && status != 404) throw new AssertionError("Delete order returned " + status);
    }

    public void deleteProduct(int id) {
        int status = given().headers(headers())
                .delete(Environment.BASE_URL + "/api/products/" + id + "?force=true").statusCode();
        if (status != 200 && status != 404) throw new AssertionError("Delete product returned " + status);
    }
