...
This briefing governs API tests in `qa-academy-exercises`. It restates, from the test-author side,
the traffic policy that QA Academy Platform enforces; that policy lives in the platform repository,
not here.
...
- The complete API command lists 39 executions: 38 extended executions and one Book execution.
  ...
  environment, public users must select one test or a maximum batch of three tests by
  traceability ID.
