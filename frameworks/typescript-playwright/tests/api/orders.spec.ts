...
test('[API-ORDER-002] Order status accepts idempotency and rejects invalid transition', async ({ request, orderService }, testInfo) => {
  ...
  try {
    ...
    const same = await step('When: client repeats the completed status', () =>
      guardedApi(testInfo, () => request.patch(`/api/orders/${controlled!.order.id}/status`, {
    ...
    await step('Then: invalid transition is rejected and status persists', async () => {
      ...
      expect((await expectApiJson<any>(invalid)).current_status).toBe('completed');
...
test('[API-ORDER-003] Payment status update persists on subsequent read', async ({ request, orderService }, testInfo) => {
  ...
  try {
    ...
    const response = await step('When: client changes payment to pending', () =>
      guardedApi(testInfo, () => request.patch(`/api/orders/${controlled!.order.id}`, {
        headers: auth(), data: { payment_status: 'pending' },
      })));
    expect(response.status()).toBe(200);
    expect((await expectApiJson<any>(response)).data.payment_status).toBe('pending');
...
test('[API-ORDER-004] Cancelling a completed order is safely rejected without changing stock', async ({ request, orderService, productService }, testInfo) => {
...
test('[API-ORDER-005] Deleting an order restores stock and removes the resource', async ({ request, orderService, productService }, testInfo) => {
  ...
  try {
    ...
    const missing = await guardedApi(testInfo, () =>
      request.get(`/api/orders/${controlled!.order.id}`, { headers: auth() }));
...
test('[API-ORDER-006] Order search returns filtered summary counts and total amount', async ({ request, orderService }, testInfo) => {
...
test('[API-ORDER-007] Relational search is applied before pagination totals', async ({ request, orderService }, testInfo) => {
...
test('[API-ORDER-008] Order resources do not disclose foreign or invalid IDs', async ({ request }, testInfo) => {
  ...
  const response = await step('When: client requests an ID outside its resource set', () =>
    guardedApi(testInfo, () => request.get('/api/orders/2147483647', { headers: auth() })));
