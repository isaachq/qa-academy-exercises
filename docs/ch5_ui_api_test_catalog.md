...
>
> All four projects implement the full catalog: 73 extended scenarios plus 4 Book executions, for 77 executions each.
...
- **`UI-QUERY-004` Depth Note**: All 4 frameworks declare the same 73 IDs and the same number of tests. Playwright (`page.route()`) and Cypress (`cy.intercept()`) intercept browser traffic and walk four server states inside the single `UI-QUERY-004` block. Selenium WebDriver has no in-process equivalent, so the Java version of that same test asserts the one state it can trigger deterministically with real data: the empty result. The difference is depth inside one test, not test count.
...
| Project | Book Executions | Extended UI | Extended API | Total Test Executions |
|---|---:|---:|---:|---:|
| **`typescript-playwright`** | 4 | 36 | 37 | **77** |
| **`python-playwright`** | 4 | 36 | 37 | **77** |
| **`java-selenium-rest-assured`** | 4 | 36 | 37 | **77** |
| **`typescript-cypress`** | 4 | 36 | 37 | **77** |

The four Book executions come from three scenarios: `BOOK-TEST-UI-001` runs on desktop and again on a Pixel 7 profile.
